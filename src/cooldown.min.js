...
this.sideLength,"' height='",this.sideLength,"'></canvas>"].join("")),this.canvasElement=this.find("canvas")[0],this.context=this.canvasElement.getContext("2d");this.css("position","relative").append(["<div class='remaining-time' style='line-height: ",this.sideLength,"px;'></div>"].join(""));this.remainingTimeElement=this.find(".remaining-time").css(this.countdownCss);var d=new Date,e=setInterval(function(){var b=(new Date-d)/a.introDuration;1<b?clearInterval(e):(a.canvasElement.width=a.canvasElement.width,
a.context.lineWidth=a.arcWidth,a.context.strokeStyle=a.arcBackgroundColor,a.context.beginPath(),a.context.arc(a.sideLength/2,a.sideLength/2,a.sideLength/2-a.arcWidth/2,Math.PI/-2,Math.PI/-2+2*Math.PI*b),a.context.stroke())},this.tickFrequency);setTimeout(function(){a.state="paused";a.remainingTimeElement.html(a.duration.toFixed(a.toFixed));a.resume.apply(a)},this.introDuration)},stop:function(){this.state="stopped";this._setActive(!1).empty()},pause:function(){"playing"===this.state&&(this.state=
